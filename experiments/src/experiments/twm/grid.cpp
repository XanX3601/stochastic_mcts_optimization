...
::twm::Problem* experiments::twm::generate_problem_1(int grid_size, int team_count) {
    ...
    std::vector<int> each_cell_initial_fuel_amount(
        grid_size * grid_size, std::floor(grid_size / (2 * neighbors_ignition_probability)));
    ...
    for (int y = 0; y < grid_size; ++y) {
        for (int x = 0; x < grid_size; ++x) {
            each_cell_burning_reward[cell_index] =
                -1 * (std::abs(x - bottom_left_corner_x) + std::abs(y - bottom_left_corner_y) + 1);
            cell_index++;
        }
    }
    each_cell_burning_reward[grid_size - 1] = -10;

    return new ::twm::Problem(grid_width, grid_height, each_cell_initial_fuel_amount,
