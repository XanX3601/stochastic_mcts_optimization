        ...
        THEN("The resulting problem must correspond to what is expected") {
            ...
            for (int cell_index = 0; cell_index < problem.get_cell_count(); ++cell_index) {
                ...
                if (problem.index_to_cell(cell_index) == bottom_left_corner) {
                    ...
                    REQUIRE_FALSE(problem.is_cell_initially_burning(cell_index));
                }

                REQUIRE(problem.get_probability_to_extinguish_cell(cell_index) == Approx(.8));

                if (cell == top_right_corner) {
                    REQUIRE(problem.get_cell_burning_reward(cell_index) == -10);
                }
                else {
                    int manhattan_distance_to_bottom_left_corner = std::abs(cell.x - bottom_left_corner.x) + std::abs(cell.y - bottom_left_corner.y);
                    REQUIRE(problem.get_cell_burning_reward(cell_index) == -1 * manhattan_distance_to_bottom_left_corner - 1);
                }

                for (int other_cell_index = 0; other_cell_index < problem.get_cell_count(); ++other_cell_index) {
                    if (cell_index == other_cell_index) {
                        REQUIRE(problem.get_probability_that_y_ignites_x(cell_index, other_cell_index) == Approx(1));
                    }
                    else if (cell.is_neighbor(problem.index_to_cell(other_cell_index))) {
                        REQUIRE(problem.get_probability_that_y_ignites_x(cell_index, other_cell_index) == Approx(.06));
                    }
                    else {
                        REQUIRE(problem.get_probability_that_y_ignites_x(cell_index, other_cell_index) == 0);
                    }
                }
            }           
        }
