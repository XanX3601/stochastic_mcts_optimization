...
TWD::Problem::Problem(int grid_width, int grid_height) : grid_width(grid_width), grid_height(grid_height) {
...
int TWD::Problem::cell_to_index(const TWD::Cell& cell) const {
    return cell.y * grid_width + cell.x;
}

TWD::Cell TWD::Problem::index_to_cell(int cell_index) const {
    return TWD::Cell(cell_index % grid_width, cell_index / grid_width);
}

int TWD::Problem::get_grid_width() const {
    return grid_width;
}

int TWD::Problem::get_grid_height() const {
    return grid_height;
}

int TWD::Problem::get_cell_count() const {
    return grid_width * grid_height;
}

int TWD::Problem::get_cell_initial_fuel_amount(const TWD::Cell& cell) const {
    return get_cell_initial_fuel_amount(cell_to_index(cell));
}

int TWD::Problem::get_cell_initial_fuel_amount(int cell_index) const {
    return each_cell_initial_fuel_amount[cell_index];
}

bool TWD::Problem::is_cell_initially_burning(const TWD::Cell& cell) const {
    return is_cell_initially_burning(cell_to_index(cell));
}

bool TWD::Problem::is_cell_initially_burning(int cell_index) const {
    return is_each_cell_initially_burning[cell_index];
}

double TWD::Problem::get_probability_that_y_ignites_x(const TWD::Cell& x, const TWD::Cell& y) const {
    return get_probability_that_y_ignites_x(cell_to_index(x), cell_to_index(y));
}

double TWD::Problem::get_probability_that_y_ignites_x(int x_index, int y_index) const {
    return each_cell_ignition_probabilities[x_index][y_index];
}

double TWD::Problem::get_probability_to_extinguish_cell(const TWD::Cell& cell) const {
    return get_probability_to_extinguish_cell(cell_to_index(cell));
}

double TWD::Problem::get_probability_to_extinguish_cell(int cell_index) const {
    return each_cell_extinguishment_probability[cell_index];
}

int TWD::Problem::get_cell_burning_reward(const TWD::Cell& cell) const {
    return get_cell_burning_reward(cell_to_index(cell));
}

int TWD::Problem::get_cell_burning_reward(int cell_index) const {
    return each_cell_burning_reward[cell_index];
}

int TWD::Problem::get_team_count() const {
    return team_count;
}

void TWD::Problem::init_random_bitstrings() {
...
unsigned long long TWD::Problem::get_cell_fuel_random_bitstring(const TWD::Cell& cell, int fuel_amount) const {
    return get_cell_fuel_random_bitstring(cell_to_index(cell), fuel_amount);
}

unsigned long long TWD::Problem::get_cell_fuel_random_bitstring(int cell_index, int fuel_amount) const {
    return each_cell_fuel_random_bitstrings.at(cell_index)[fuel_amount];
}

unsigned long long TWD::Problem::get_cell_burning_random_bitstring(const TWD::Cell& cell, bool is_cell_burning) const {
    return get_cell_burning_random_bitstring(cell_to_index(cell), is_cell_burning);
}

unsigned long long TWD::Problem::get_cell_burning_random_bitstring(int cell_index, bool is_cell_burning) const {
    return each_cell_burning_random_bitstrings.at(cell_index)[is_cell_burning];
}

unsigned long long TWD::Problem::get_cell_team_random_bitstring(const TWD::Cell& cell, int team_index) const {
    return get_cell_team_random_bitstring(cell_to_index(cell), team_index);
}

unsigned long long TWD::Problem::get_cell_team_random_bitstring(int cell_index, int team_index) const {
    return each_cell_team_random_bitstrings.at(cell_index)[team_index];
}

unsigned long long TWD::Problem::get_team_turn_random_bitstring(int team_index) const {
